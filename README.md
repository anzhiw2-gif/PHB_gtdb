...
| 参数 | 值 | 说明 |
|------|-----|------|
| 模型选择 | `-m MFP` | ModelFinder Plus 自动选择最优氨基酸替代模型 |
| 分支支持 | `-B 1000` | 标准 bootstrap + SH-aLRT (IQ-TREE 默认双输出) |
| 线程 | 8–30 | 按亚型大小分配 |

### 改进计划

详见 [`docs/REVIEW.md`](docs/REVIEW.md) — 管线评审与改进路线图:
- 🔴 HMMer Pfam 二次验证 + 关键位点检测 + SignalP
- 🟡 负对照 + 多拷贝统计 + Logo 图 + 联合树
- 🟢 AlphaFold2 + reconciliation + 底物特异性
